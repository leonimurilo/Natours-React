...
[Getting "Error watching file for changes: EMFILE" ?](#watchman-error)
...
<a name="watchman-error"></a>
### Getting "Error watching file for changes: EMFILE"?
If you are using macOS High Sierra or newer, run "brew install watchman";
...
| test | Runs tests (files ending in .test.js or .test.js) using Jest and outputs results to the command line. Watches all files so tests are re-run upon save. |
...
Streamlined automated testing is a core feature of this starter kit. All tests are placed in files that end in .test.js. Spec files are placed in the same directory as the file under test. Why?
